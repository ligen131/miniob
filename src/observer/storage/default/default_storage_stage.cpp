...
void DefaultStorageStage::handle_event(StageEvent *event) {
  ...
  case SCF_INSERT: { // insert into
      ...
      rc = handler_->insert_record(current_trx, current_db, table_name, inserts.value_num, inserts.values, inserts.data_num , inserts.data_list_r);
  ...
  case SCF_CREATE_INDEX: {
      ...
      rc = handler_->create_index(current_trx, current_db, create_index.relation_name, 
                                  create_index.index_name, (const char **)create_index.attribute_name, create_index.attr_num, create_index._is_unique);
      snprintf(response, sizeof(response), "%s\n", rc == RC::SUCCESS ? "SUCCESS" : "FAILURE");
    }
    break;
...
RC insert_record_from_file(Table *table, std::vector<std::string> &file_values, 
  ...
  if (RC::SUCCESS == rc) {
    int data_list_r[]={field_num};
    rc = table->insert_record(nullptr, field_num, record_values.data(), 1, data_list_r);
    if (rc != RC::SUCCESS) {
